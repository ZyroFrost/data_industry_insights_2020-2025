...
def clean_value(value, column_name=""):
    ...
    if isinstance(value, str):
        ...
        # CRITICAL: Marker values phải thành NULL
...
def get_db_connection():
    try:
        ...
        cur = conn.cursor()
        #cur.execute("SET search_path TO data_industry_insights, public;") # dòng này chỉ dùng up lên supabase
        conn.commit()
        cur.close()
...
def load_existing_ids(conn, id_column: str, table_name: str) -> set:
    """Load tất cả ID đã tồn tại trong DB - KHÔNG DÙNG NỮA"""
    # Function này giữ lại để tránh lỗi, nhưng không dùng
    return set()
...
def load_existing_id_mapping(conn, id_column: str, table_name: str):
    ...
    try:
        ...
        # new_id = old_id vì không truncate
        for row in rows:
            new_id = row[0]
...
def load_csv_to_db_optimized(conn, csv_file: str, table_name: str, id_column: str, 
    ...
    # Check CSV file có bao nhiêu rows
    ...
    if choice != 'y':
        ...
        # RETURN NGAY - không xử lý CSV nữa
        return True
    else:
        # Truncate bảng
    ...
    # Đến đây nghĩa là user chọn Y - tiếp tục load CSV
    existing_ids = set()  # Không cần check duplicate vì đã truncate
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                # Không cần check existing_ids vì đã truncate rồi
                values = [clean_value(row[col], col) for col in columns]
                
                # CRITICAL: Chỉ skip nếu thiếu data BẮT BUỘC
                # companies: company_name bắt buộc
                if table_name == "companies" and values[0] is None:
                    skipped_count += 1
                    continue
                
                # locations: city cho phép NULL, không skip
                # role_names: role_name bắt buộc
                ...
                # Removed duplicate validation - đã check ở trên rồi
...
def load_csv_with_fk_mapping_optimized(conn, csv_file: str, table_name: str, 
    ...
    print(f"\n📋 Bảng: {table_name}")
    print(f"   Hiện tại: {current_count:,} rows")
    ...
    # Đến đây nghĩa là user chọn Y - tiếp tục load CSV
    existing_pairs = set()  # Không cần check duplicate vì đã truncate
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                # Không cần check existing_pairs vì đã truncate rồi
                data_batch.append(values)
